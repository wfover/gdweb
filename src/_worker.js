...
var _status, _executionCtx, _headers, _preparedHeaders, _res, _isFresh, _a, _validatedData, _matchResult, _b, _path, _c;
...
var Context = (_a = class {
  constructor(req, options) {
    ...
    __publicField(this, "setLayout", (layout) => this.layout = layout);
    __publicField(this, "getLayout", () => this.layout);
...
}, _status = new WeakMap(), _executionCtx = new WeakMap(), _headers = new WeakMap(), _preparedHeaders = new WeakMap(), _res = new WeakMap(), _isFresh = new WeakMap(), _a);
...
}, _validatedData = new WeakMap(), _matchResult = new WeakMap(), _b);
...
var Trie = class {
  constructor() {
    ...
    __publicField(this, "root", new Node$1());
  }
...
var ConfigKey = /* @__PURE__ */ ((ConfigKey2) => {
  ConfigKey2["shouldShowRecent"] = "config/should_show_recent";
  ConfigKey2["aiTag"] = "config/ai_tag";
  return ConfigKey2;
})(ConfigKey || {});
function hashToken(token) {
  return createHash("sha256").update(token).digest("hex");
}
async function checkAdminExist(DB) {
  const result2 = await DB.prepare(`SELECT COUNT(*) as count FROM stores WHERE key = 'ADMIN_TOKEN'`).first();
...
async function verifyAdminToken(DB, token) {
  ...
  const result2 = await DB.prepare(`SELECT COUNT(*) as count FROM stores WHERE key = 'ADMIN_TOKEN' AND value = ?`).bind(token).first();
  if (result2.count > 0) {
    return "accept";
  }
...
async function setAdminToken(DB, token) {
  const exist = await checkAdminExist(DB);
  if (exist) {
    throw new Error("Admin token already exists");
  }
  const result2 = await DB.prepare(`INSERT INTO stores (key, value) VALUES ('ADMIN_TOKEN', ?)`).bind(token).run();
  return result2.success;
}
async function getShouldShowRecent(DB) {
  const result2 = await DB.prepare(`SELECT value FROM stores WHERE key = '${ConfigKey.shouldShowRecent}'`).all();
  if (!result2.success) {
    throw result2.error;
  }
  ...
  return result2.results[0].value === "true";
}
async function setShouldShowRecent(DB, value) {
  ...
  const result2 = await DB.prepare(insertSql).bind(bindValue, bindValue).run();
  if (!result2.success) {
    throw result2.error;
  }
  return result2.success;
}
async function getAITagConfig(DB) {
  const result2 = await DB.prepare(`SELECT value FROM stores WHERE key = '${ConfigKey.aiTag}'`).all();
  if (!result2.success) {
    throw result2.error;
  }
  if (result2.results.length === 0) {
  ...
  return JSON.parse(result2.results[0].value);
}
async function setAITagConfig(DB, config) {
...
async function tokenMiddleware(c, next) {
  ...
  const result2 = await verifyAdminToken(c.env.DB, token);
  if (result2 === "reject") {
    return c.json({ error: "Invalid token" }, 401);
  }
  if (result2 === "fail") {
...
function buildGenerateTagMessage(props) {
  return [
    {
      role: "system",
...
function generateChatCompletion(tagLanguage, preferredTags) {
...
async function selectAllTags(DB) {
  const sql = `
    SELECT 
      *
    FROM tags
  ...
  const tagList = sqlResult.results.map((tag) => {
    const pageIdDict = JSON.parse(tag.pageIdDict);
    return {
      ...tag,
...
async function insertTag(DB, options) {
...
async function updateTag(DB, options) {
...
async function deleteTagById(DB, id) {
  const sql = `
    DELETE FROM tags
    WHERE id = ?
  `;
...
function generateUpdateTagSql(DB, bindList, unbindList) {
...
async function updateBindPageByTagName(DB, bindList, unbindList) {
  const commands = generateUpdateTagSql(DB, bindList, unbindList);
  if (commands.length === 0) {
    return true;
  }
  const updateResult = await DB.batch(commands);
  return updateResult.every((result2) => result2.success);
}
function pageIdsToBindDictString(pageIds) {
...
function pageIdsToUnbindDictString(pageIds) {
  const dict = pageIds.reduce((acc, cur) => {
...
async function formFileToArrayBuffer(file) {
...
async function removeBucketFile(BUCKET, ids) {
  if (isNil(ids)) {
    return;
  }
  await BUCKET.delete(ids);
}
async function saveFileToBucket(BUCKET, file) {
  if (isNil(file)) {
    return;
  }
...
async function getFileFromBucket(BUCKET, id) {
  return await BUCKET.get(id);
}
async function getBase64FileFromBucket(BUCKET, id, type) {
...
async function blobToBase64(blob, type) {
  const buffer = await blob.arrayBuffer();
  const base64String = Buffer2.from(buffer).toString("base64");
  return `data:${type};base64,${base64String}`;
}
async function selectPageTotalCount(DB, options) {
  ...
  const result2 = await DB.prepare(sql).bind(...bindParams).first();
  return result2.count;
}
async function selectAllPageCount(DB) {
  const sql = `
    SELECT COUNT(*) as count FROM pages
    WHERE isDeleted = 0
  `;
  const result2 = await DB.prepare(sql).first();
  return result2.count;
}
async function queryPage(DB, options) {
  ...
  const sqlResult = await DB.prepare(sql).bind(...bindParams).all();
  if (sqlResult.error) {
    throw sqlResult.error;
  }
  return sqlResult.results;
}
async function queryDeletedPage(DB) {
  const sql = `
    SELECT
      id,
      title,
  ...
  `;
  const result2 = await DB.prepare(sql).all();
  return result2.results;
}
async function deletePageById(DB, pageId) {
  const sql = `
    UPDATE pages
    SET 
      isDeleted = 1,
    ...
    WHERE id = ?
  `;
  const result2 = await DB.prepare(sql).bind(pageId).run();
  return result2.success;
}
async function restorePage(DB, id) {
  const sql = `
    UPDATE pages
    SET 
      isDeleted = 0,
    ...
    WHERE id = ?
  `;
  const result2 = await DB.prepare(sql).bind(id).run();
  return result2.success && result2.meta.changes > 0;
}
async function getPageById(DB, options) {
  ...
  const sql = `
    ...
    WHERE id = ?
  `;
  const page = await DB.prepare(sql).bind(id).first();
  if (isNotNil(isDeleted) && (page == null ? void 0 : page.isDeleted) !== Number(isDeleted)) {
    return null;
  }
  return page;
}
async function insertPage(DB, pageOptions) {
  ...
  ).bind(title, pageDesc, pageUrl, contentUrl, folderId, screenshotId, isShowcased).run();
  return insertResult.meta.last_row_id;
}
async function clearDeletedPage(DB, BUCKET) {
  const pageListSql = `
    SELECT * FROM pages WHERE isDeleted = 1
  `;
  const deletePageResult = await DB.prepare(pageListSql).all();
  if (deletePageResult.error) {
    return false;
  }
  const deleteBucketKeys = deletePageResult.results.map((page) => [page.screenshotId, page.contentUrl]).flat().filter(isNotNil);
  await removeBucketFile(BUCKET, deleteBucketKeys);
  const sql = `
    DELETE FROM pages WHERE isDeleted = 1
  `;
  const result2 = await DB.prepare(sql).run();
  return result2.success;
}
async function queryRecentSavePage(DB) {
  const sql = `
...
async function updatePage(DB, options) {
  ...
  const result2 = await DB.batch([updateSql, ...updateSqlList]);
  return result2.every((r) => r.success);
}
async function checkFolderExists(DB, name) {
  const sql = `
    SELECT 
      id
    FROM folders
...
async function insertFolder(DB, name) {
  const sql = `
...
async function updateFolder(DB, options) {
  const sql = `
    UPDATE folders
    SET name = ?
    WHERE id = ?
...
async function selectAllFolders(DB) {
  const sql = `
    SELECT 
      *
    FROM folders
...
async function deleteFolderById(DB, id) {
...
async function getFolderById(DB, options) {
  const { id, isDeleted } = options;
  const sql = `
    SELECT 
      *
    ...
    WHERE id = ?
  `;
  const folder = await DB.prepare(sql).bind(id).first();
  if (isNotNil(isDeleted) && (folder == null ? void 0 : folder.isDeleted) !== Number(isDeleted)) {
    return null;
  }
  return folder;
}
async function queryDeletedFolders(DB) {
  const sql = `
    SELECT 
      *
    FROM folders
...
async function restoreFolder(DB, id) {
  const sql = `
    UPDATE folders
    SET 
      isDeleted = 0,
...
async function getHomeChartData(DB) {
  ...
  const sortedFolderPageCountList = folderPageCountList.sort((a, b) => b.pageCount - a.pageCount).slice(0, 5);
...
async function queryShowcase(DB, options) {
  ...
  const [sqlResult, countResult] = await Promise.all([
    DB.prepare(querySql).bind(pageSize, (pageNumber - 1) * pageSize).all(),
    DB.prepare(countSql).first()
  ]);
  return {
...
async function updateShowcase(DB, options) {
  const { id, isShowcased } = options;
  const sql = `
    UPDATE pages SET isShowcased = ? WHERE id = ?
  `;
  const result2 = await DB.prepare(sql).bind(isShowcased, id).run();
  return result2.success;
}
async function getShowcaseDetailById(DB, options) {
  const { id } = options;
  const sql = `
    SELECT 
      *
...
async function getNextShowcasePageId(DB, lastId) {
  const sql = `
    SELECT id
    FROM pages
    WHERE isShowcased = 1 AND isDeleted = 0 AND id > ?
    ...
    LIMIT 1
  `;
  const result2 = await DB.prepare(sql).bind(lastId).first();
  if (result2) {
    return result2.id;
  }
  const firstShowcaseSql = `
    ...
    LIMIT 1
  `;
  const firstResult = await DB.prepare(firstShowcaseSql).first();
...
app$6.get("/content", async (c) => {
  var _a2;
  const pageId = c.req.query("pageId");
  if (!pageId) {
    return c.redirect("/error");
  }
  const pageListResult = await c.env.DB.prepare("SELECT * FROM pages WHERE isShowcased = 1 AND isDeleted = 0 AND id = ?").bind(pageId).all();
  if (!pageListResult.success) {
    return c.redirect("/error");
  }
  const page = (_a2 = pageListResult.results) == null ? void 0 : _a2[0];
  if (!page) {
    return c.redirect("/error");
  }
  ...
  return c.html(
    await (content == null ? void 0 : content.text())
  );
});
app$6.get(
  "/detail",
...
app$5.post(
  ...
  validator("json", (value, c) => {
    ...
    return {
      folderId: isNotNil(value.folderId) ? Number(value.folderId) : void 0,
      tagId: isNotNil(value.tagId) ? Number(value.tagId) : void 0,
      pageNumber: isNotNil(value.pageNumber) ? Number(value.pageNumber) : void 0,
      pageSize: isNotNil(value.pageSize) ? Number(value.pageSize) : void 0,
...
app$5.put(
  ...
  validator("json", (value, c) => {
    ...
    return {
      ...
      pageDesc: value.pageDesc ?? "",
      pageUrl: value.pageUrl ?? "",
      bindTags: value.bindTags ?? [],
      unbindTags: value.unbindTags ?? []
    };
  }),
  async (c) => {
...
app$1.get("/should_show_recent", async (c) => {
  try {
...
app$1.post(
  ...
  validator("json", (value, c) => {
    ...
    return {
      shouldShowRecent: value.shouldShowRecent
    };
  }),
  async (c) => {
...
const api = new Hono();
api.route("/showcase", app$6);
api.use(tokenMiddleware);
api.route("/pages", app$5);
api.route("/auth", app$4);
api.route("/folders", app$3);
api.route("/tags", app$2);
api.route("/data", app$7);
api.route("/config", app$1);
app.route("/api", api);
const worker = new Hono();
const modules = /* @__PURE__ */ Object.assign({ "/src/server.ts": app });
let added = false;
for (const [, app2] of Object.entries(modules)) {
  if (app2) {
    worker.route("/", app2);
    worker.notFound(app2.notFoundHandler);
    added = true;
  }
}
if (!added) {
  throw new Error("Can't import modules from ['/src/server.ts']");
}
export {
  worker as default
};
